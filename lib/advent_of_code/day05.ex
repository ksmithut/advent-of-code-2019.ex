defmodule AdventOfCode.Day05 do
  ...
  ## Examples
      ...
      iex> AdventOfCode.Day05.part1("1002,4,3,4,33")
      nil
  ...
  def run_program(state, pos, input \\ nil, output \\ nil) do
    ...
    cond do
      ...
      opcode == "01" ->
        List.replace_at(state, cIm, a + b) |> run_program(pos + 4, input, output)

      opcode == "02" ->
        List.replace_at(state, cIm, a * b) |> run_program(pos + 4, input, output)

      opcode == "03" ->
        List.replace_at(state, aIm, input) |> run_program(pos + 2, input, output)

      opcode == "04" ->
        run_program(state, pos + 2, nil, a)

      opcode == "05" and a != 0 ->
        run_program(state, b, input, output)

      opcode == "05" ->
        run_program(state, pos + 3, input, output)
      ...
      opcode == "07" and a < b ->
        List.replace_at(state, cIm, 1) |> run_program(pos + 4, input, output)

      opcode == "07" ->
        List.replace_at(state, cIm, 0) |> run_program(pos + 4, input, output)
